...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,                //extended window style          
            ...
            TEXT("Direct3D11 : Smiley"),                  //window caption
...
HRESULT Initialize(void)
{
    ...
    HRESULT loadD3DTexture(ID3D11ShaderResourceView **ppID3D11ShaderResourceView, TCHAR ResourceID[]);
    ...
    ZeroMemory((void*)&d3d11BufferDesc, sizeof(D3D11_BUFFER_DESC));
    d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(square_texcoords);
    d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    d3d11BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    d3d11BufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    ...
    {
        fopen_s(&gpFile, gLogFileName, "a+");
        fprintf(gpFile, "Error : ID3D11Device::CreateBuffer() failed for texcoord buffer.\n");
        fclose(gpFile);
        return (hr);
    }
    ...
    //load texture 
    hr = loadD3DTexture(&gpID3D11ShaderResourceView_texture, MAKEINTRESOURCE(SMILEY_BITMAP));
    if(FAILED(hr))
    {
        fopen_s(&gpFile, gLogFileName, "a+");
        fprintf(gpFile, "Error : loadD3DTexture() failed to load Smiley.bmp\n");
        fclose(gpFile);
        //return (hr);
    }
...
HRESULT loadD3DTexture(ID3D11ShaderResourceView **ppID3D11ShaderResourceView, TCHAR ResourceID[])
{
    ...
    ID3D11Resource *pID3D11Resource_texture = NULL;
    ...
    if(hBitmap)
    {
        ...
        hr = DirectX::CreateWICTextureFromMemory(gpID3D11Device, 
                                                 gpID3D11DeviceContext,
                                                 (uint8_t*)bmp.bmBits, 
                                                 bmp.bmWidth * bmp.bmHeight * bmp.bmPlanes,
                                                 NULL,
