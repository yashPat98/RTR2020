...
float triangle_rotation_angle;
float rectangle_rotation_angle;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,                //extended window style          
            ...
            TEXT("Direct3D11 : 2D Animation"),            //window caption
...
HRESULT Initialize(void)
{
    ...
    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_triangle_position);
    if(FAILED(hr))
    {
        fopen_s(&gpFile, gLogFileName, "a+");
    ...
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_triangle_position, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3d11MappedSubresource);
    ...
    //create vertex buffer for colors of triangle
    ZeroMemory((void*)&d3d11BufferDesc, sizeof(D3D11_BUFFER_DESC));
    d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(triangle_colors);
    d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    d3d11BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    d3d11BufferDesc.Usage = D3D11_USAGE_DYNAMIC;

    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_triangle_color);
    if(FAILED(hr))
    {
        fopen_s(&gpFile, gLogFileName, "a+");
        fprintf(gpFile, "Error : ID3D11Device::CreateBuffer() failed for vertex buffer.\n");
    ...
    ZeroMemory((void*)&d3d11MappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_triangle_color, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3d11MappedSubresource);
    memcpy(d3d11MappedSubresource.pData, triangle_colors, sizeof(triangle_colors));
    ...
    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_rectangle_position);
    if(FAILED(hr))
    {
        fopen_s(&gpFile, gLogFileName, "a+");
        fprintf(gpFile, "Error : ID3D11Device::CreateBuffer() failed for vertex buffer.\n");
    ...
    ZeroMemory((void*)&d3d11MappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_rectangle_position, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3d11MappedSubresource);
    memcpy(d3d11MappedSubresource.pData, rectangle_vertices, sizeof(rectangle_vertices));
    gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_rectangle_position, 0);

    //create vertex buffer for colors of rectangle
    ZeroMemory((void*)&d3d11BufferDesc, sizeof(D3D11_BUFFER_DESC));
    d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(rectangle_colors);
    d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    d3d11BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    d3d11BufferDesc.Usage = D3D11_USAGE_DYNAMIC;

    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_rectangle_color);
    if(FAILED(hr))
    {
        fopen_s(&gpFile, gLogFileName, "a+");
        fprintf(gpFile, "Error : ID3D11Device::CreateBuffer() failed for vertex buffer.\n");
    ...
    ZeroMemory((void*)&d3d11MappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_rectangle_color, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3d11MappedSubresource);
    memcpy(d3d11MappedSubresource.pData, rectangle_colors, sizeof(rectangle_colors));
    gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_rectangle_color, 0);
...
void Display(void)
{
    ...
    rotationMatrix = XMMatrixRotationY(XMConvertToRadians(-triangle_rotation_angle));
    ...
    //rectangle
    translationMatrix = XMMatrixTranslation(1.5f, 0.0f, 6.0f);
    rotationMatrix = XMMatrixRotationX(XMConvertToRadians(-rectangle_rotation_angle));
    worldMatrix = rotationMatrix * translationMatrix;
    viewMatrix = XMMatrixIdentity();
    wvpMatrix = worldMatrix * viewMatrix * perspectiveProjectionMatrix;
    ...
    stride = sizeof(float) * 3;
    offset = 0;
    gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBuffer_rectangle_position, &stride, &offset);
    
    stride = sizeof(float) * 3;
    offset = 0;
    gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_rectangle_color, &stride, &offset);
    ...
    rectangle_rotation_angle += 0.05f;
    if(rectangle_rotation_angle >= 360.0f)
        rectangle_rotation_angle = 0.0f;
