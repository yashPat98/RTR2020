...
struct CBUFFER
{
    ...
    unsigned int ToggleLight;
    unsigned int ToggleTexture;
};
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,                //extended window style          
            ...
            TEXT("Direct3D11 : Interleaved"),             //window caption
...
HRESULT Initialize(void)
{
    ...
    const char *vertexShaderSourceCode = 
        ...
        "vertex_output main(float3 pos:POSITION, float3 color:COLOR, float3 normal:NORMAL, float2 texcoord:TEXCOORD)"       \
        "{"                                                                                                                 \
        ...
        "   if(toggleLight == 1)"                                                                                           \
        "   {"                                                                                                              \
        "       float4 eye_position = mul(worldViewMatrix, float4(pos, 1.0f));"                                             \
        "       float3 tnorm = normalize(mul((float3x3)worldViewMatrix, (float3)normal));"                                  \
        "       float3 s = (float3)normalize(lightPosition - eye_position);"                                                \
        "       output.color = lightDiffuse * output.color * max(dot(s, tnorm), 0.0f);"                                     \
        "   }"                                                                                                              \

        "   output.texcoord = texcoord;"                                                                                    \
        "   output.position = mul(projectionMatrix, mul(worldViewMatrix, float4(pos, 1.0f)));"                              \
    ...
    const char *pixelShaderSourceCode = 
        ...
        "{"                                                                                         \
        "   float4 pixel_color = input.color;"                                                      \
        
        "   if(toggleTexture == 1)"                                                                 \
        "   {"                                                                                      \
        "       pixel_color = pixel_color * myTexture2D.Sample(mySamplerState, input.texcoord);"    \
        "   }"                                                                                      \
    ...
    d3d11InputElementDesc[2].SemanticName = "NORMAL";
    ...
    d3d11InputElementDesc[3].SemanticName = "TEXCOORD";
    ...
    d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cube_pcnt);
    ...
    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_cube);
    if(FAILED(hr))
    {
        fopen_s(&gpFile, gLogFileName, "a+");
    ...
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_cube, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3d11MappedSubresource);
    memcpy(d3d11MappedSubresource.pData, cube_pcnt, sizeof(cube_pcnt));
    gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_cube, 0);
    ...
    {
        fopen_s(&gpFile, gLogFileName, "a+");
        fprintf(gpFile, "Error : loadD3DTexture() failed to load marble.bmp\n");
        fclose(gpFile);
        return (hr);
    }
...
void Display(void)
{
    ...
    worldMatrix = XMMatrixRotationX(XMConvertToRadians(-cube_rotation_angle));
    worldMatrix = worldMatrix * XMMatrixRotationY(XMConvertToRadians(-cube_rotation_angle));
    worldMatrix = worldMatrix * XMMatrixRotationZ(XMConvertToRadians(-cube_rotation_angle));
    worldMatrix = worldMatrix * XMMatrixScaling(0.75f, 0.75f, 0.75f);
    worldMatrix = worldMatrix * XMMatrixTranslation(0.0f, 0.0f, 6.0f);

    viewMatrix = XMMatrixIdentity();

    ZeroMemory((void*)&constantBuffer, sizeof(CBUFFER));
    constantBuffer.WorldViewMatrix = viewMatrix * worldMatrix;
    constantBuffer.ProjectionMatrix = perspectiveProjectionMatrix;

    if(gbColor == true)
    {
        constantBuffer.ToggleColor = 1;
    }
    else 
    {
        constantBuffer.ToggleColor = 0;
    }

    if(gbLight == true)
    {
        constantBuffer.ToggleLight = 1;
        constantBuffer.LightDiffuse = XMVectorSet(1.0f, 1.0f, 1.0f, 1.0f);
        constantBuffer.LightPosition = XMVectorSet(0.0f, 0.0f, -2.0f, 1.0f);
    }
    else 
    {
        constantBuffer.ToggleLight = 0;
    }

    if(gbTexture)
    {
        constantBuffer.ToggleTexture = 1;
    }
    else 
    {
        constantBuffer.ToggleTexture = 0;
    }
