...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,                //extended window style          
            ...
            TEXT("Direct3D11 : 3D Animation"),            //window caption
...
HRESULT Initialize(void)
{
    ...
    //create vertex buffer for vertices of pyramid
    ...
    //create vertex buffer for colors of pyramid
    ZeroMemory((void*)&d3d11BufferDesc, sizeof(D3D11_BUFFER_DESC));
    ...
    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_pyramid_color);
    if(FAILED(hr))
    {
        fopen_s(&gpFile, gLogFileName, "a+");
        fprintf(gpFile, "Error : ID3D11Device::CreateBuffer() failed for vertex buffer.\n");
    ...
    ZeroMemory((void*)&d3d11MappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_pyramid_color, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3d11MappedSubresource);
    ...
    //create vertex buffer for colors of cube
    ZeroMemory((void*)&d3d11BufferDesc, sizeof(D3D11_BUFFER_DESC));
    d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cube_colors);
    d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    d3d11BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    d3d11BufferDesc.Usage = D3D11_USAGE_DYNAMIC;

    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_cube_color);
    if(FAILED(hr))
    {
        fopen_s(&gpFile, gLogFileName, "a+");
        fprintf(gpFile, "Error : ID3D11Device::CreateBuffer() failed for vertex buffer.\n");
    ...
    ZeroMemory((void*)&d3d11MappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_cube_color, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3d11MappedSubresource);
    memcpy(d3d11MappedSubresource.pData, cube_colors, sizeof(cube_colors));
...
void Display(void)
{
    ...
    XMMATRIX xRotationMatrix;
    XMMATRIX yRotationMatrix;
    XMMATRIX zRotationMatrix;
    ...
    rotationMatrix = XMMatrixRotationY(XMConvertToRadians(-pyramid_rotation_angle));
    ...
    xRotationMatrix = XMMatrixRotationX(XMConvertToRadians(-cube_rotation_angle));
    yRotationMatrix = XMMatrixRotationY(XMConvertToRadians(-cube_rotation_angle));
    zRotationMatrix = XMMatrixRotationZ(XMConvertToRadians(-cube_rotation_angle));
