...
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_pyramid_texcoord = NULL;
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_cube_position = NULL;
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_cube_texcoord = NULL;
ID3D11InputLayout *gpID3D11InputLayout = NULL;
ID3D11Buffer *gpID3D11Buffer_ConstantBuffer = NULL;

ID3D11ShaderResourceView *gpID3D11ShaderResourceView_stone_texture = NULL;
ID3D11ShaderResourceView *gpID3D11ShaderResourceView_kundali_texture = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,                //extended window style          
            ...
            TEXT("Direct3D11 : Textured Animation"),      //window caption
...
HRESULT Initialize(void)
{
    ...
    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_pyramid_position);
    if(FAILED(hr))
    {
        fopen_s(&gpFile, gLogFileName, "a+");
        fprintf(gpFile, "Error : ID3D11Device::CreateBuffer() failed for pyramid position buffer.\n");
        fclose(gpFile);
        return (hr);
    }
    ...
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_pyramid_position, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3d11MappedSubresource);
    ...
    //create vertex buffer for pyramid texcoord
    ZeroMemory((void*)&d3d11BufferDesc, sizeof(D3D11_BUFFER_DESC));
    ...
    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_pyramid_texcoord);
    if(FAILED(hr))
    {
        fopen_s(&gpFile, gLogFileName, "a+");
        fprintf(gpFile, "Error : ID3D11Device::CreateBuffer() failed for pyramid texcoords.\n");
        fclose(gpFile);
        return (hr);
    }

    ZeroMemory((void*)&d3d11MappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_pyramid_texcoord, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3d11MappedSubresource);
    memcpy(d3d11MappedSubresource.pData, pyramid_texcoord, sizeof(pyramid_texcoord));
    gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_pyramid_texcoord, 0);

    //create vertex buffer for cube position
    ZeroMemory((void*)&d3d11BufferDesc, sizeof(D3D11_BUFFER_DESC));
    d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cube_vertices);
    d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    d3d11BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    d3d11BufferDesc.Usage = D3D11_USAGE_DYNAMIC;

    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_cube_position);
    if(FAILED(hr))
    {
        fopen_s(&gpFile, gLogFileName, "a+");
        fprintf(gpFile, "Error : ID3D11Device::CreateBuffer() failed for cube position.\n");
        fclose(gpFile);
        return (hr);
    }

    ZeroMemory((void*)&d3d11MappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_cube_position, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3d11MappedSubresource);
    memcpy(d3d11MappedSubresource.pData, cube_vertices, sizeof(cube_vertices));
    gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_cube_position, 0);

    //create vertex buffer for cube texcoords
    ZeroMemory((void*)&d3d11BufferDesc, sizeof(D3D11_BUFFER_DESC));
    d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cube_texcoords);
    d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    d3d11BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    d3d11BufferDesc.Usage = D3D11_USAGE_DYNAMIC;

    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_cube_texcoord);
    if(FAILED(hr))
    {
        fopen_s(&gpFile, gLogFileName, "a+");
        fprintf(gpFile, "Error : ID3D11Device::CreateBuffer() failed for cube texcoords.\n");
        fclose(gpFile);
        return (hr);
    }

    ZeroMemory((void*)&d3d11MappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_cube_texcoord, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3d11MappedSubresource);
    memcpy(d3d11MappedSubresource.pData, cube_texcoords, sizeof(cube_texcoords));
    gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_cube_texcoord, 0);
    ...
    //texture
    hr = loadD3DTexture(L"Stone.bmp", &gpID3D11ShaderResourceView_stone_texture);
    if(FAILED(hr))
    {
        fopen_s(&gpFile, gLogFileName, "a+");
    ...
    hr = loadD3DTexture(L"Vijay_Kundali.bmp", &gpID3D11ShaderResourceView_kundali_texture);
    if(FAILED(hr))
    {
        fopen_s(&gpFile, gLogFileName, "a+");
        fprintf(gpFile, "Error : loadD3DTexture() failed to load Vijay_Kundali.bmp\n");
        fclose(gpFile);
        return (hr);
    }
...
void Display(void)
{
    ...
    offset = 0;
    gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_pyramid_texcoord, &stride, &offset);
    ...
    rotationMatrix = XMMatrixRotationY(-pyramid_rotation_angle);
    ...
    //set sampler slots
    gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_stone_texture);
    ...
    stride = sizeof(float) * 2;
    offset = 0;
    gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_cube_texcoord, &stride, &offset);

    //set matrices 
    translationMatrix = XMMatrixTranslation(1.5f, 0.0f, 5.0f);
    rotationMatrix = XMMatrixRotationX(-pyramid_rotation_angle);
    rotationMatrix *= XMMatrixRotationY(-pyramid_rotation_angle);
    rotationMatrix *= XMMatrixRotationZ(-pyramid_rotation_angle);
    ...
    //set sampler slots
    gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_kundali_texture);
    gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpID3D11SamplerState_texture);
...
void Cleanup(void)
{
    ...
    if(gpID3D11ShaderResourceView_stone_texture)
    {
        gpID3D11ShaderResourceView_stone_texture->Release();
        gpID3D11ShaderResourceView_stone_texture = NULL;
    }

    if(gpID3D11ShaderResourceView_kundali_texture)
    {
        gpID3D11ShaderResourceView_kundali_texture->Release();
        gpID3D11ShaderResourceView_kundali_texture = NULL;
    }
    ...
    if(gpID3D11Buffer_VertexBuffer_cube_texcoord)
    {
        gpID3D11Buffer_VertexBuffer_cube_texcoord->Release();
        gpID3D11Buffer_VertexBuffer_cube_texcoord = NULL;
    }
    ...
    if(gpID3D11Buffer_VertexBuffer_pyramid_texcoord)
    {
        gpID3D11Buffer_VertexBuffer_pyramid_texcoord->Release();
        gpID3D11Buffer_VertexBuffer_pyramid_texcoord = NULL;
    }
