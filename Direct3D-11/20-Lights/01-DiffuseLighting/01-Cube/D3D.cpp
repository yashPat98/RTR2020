...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,                //extended window style          
            ...
            TEXT("Direct3D11 : Diffuse Lighting"),        //window caption
...
HRESULT Initialize(void)
{
    ...
    const char *vertexShaderSourceCode = 
        ...
        "       output.diffuse_light = ld * kd * max(dot(s, tnorm), 0.0f);"                     \
        "   }"                                                                                  \
        "   else"                                                                               \
        "   {"                                                                                  \
    ...
    ZeroMemory((void*)&d3d11BufferDesc, sizeof(D3D11_BUFFER_DESC));
    d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cube_normals);
    d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    d3d11BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    d3d11BufferDesc.Usage = D3D11_USAGE_DYNAMIC;

    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_cube_normal);
    if(FAILED(hr))
    {
        fopen_s(&gpFile, gLogFileName, "a+");
        fprintf(gpFile, "Error : ID3D11Device::CreateBuffer() failed for vertex buffer.\n");
    ...
    ZeroMemory((void*)&d3d11MappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_cube_normal, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3d11MappedSubresource);
    memcpy(d3d11MappedSubresource.pData, cube_normals, sizeof(cube_normals));
    gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_cube_normal, 0);
