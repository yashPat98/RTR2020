...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,                //extended window style          
            ...
            TEXT("Direct3D11 : Sphere"),                  //window caption
...
HRESULT Initialize(void)
{
    ...
    const char *vertexShaderSourceCode = 
        ...
        "       float3 transformed_normal = (float3)normalize(mul((float3x3)worldViewMatrix, (float3)normal));"         \
        "       float3 light_direction = (float3)normalize(lightPosition - eye_position);"                              \
        "       output.diffuse_light = ld * kd * max(dot(light_direction, transformed_normal), 0.0f);"                  \
        "   }"                                                                                                          \
        "   else"                                                                                                       \
        "   {"                                                                                                          \
