...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,                      //extended window style          
            ...
            TEXT("Direct3D11 : Three Moving Lights"),           //window caption
...
HRESULT Initialize(void)
{
    ...
    const char *vertexShaderSourceCode_perVertexLighting = 
        ...
        "       for(int i = 0; i < 3; i++)"                                                                                     \
        "       {"                                                                                                              \
        "           float3 light_direction = (float3)normalize(lightPosition[i] - eye_position);"                               \
        "           float3 reflection_vector = reflect(-light_direction, transformed_normal);"                                  \

        "           float3 ambient = la[i] * ka;"                                                                               \
        "           float3 diffuse = ld[i] * kd * max(dot(light_direction, transformed_normal), 0.0f);"                         \
        "           float3 specular = ls[i] * ks * pow(max(dot(reflection_vector, viewer_vector), 0.0f), materialShininess);"   \
        
        "           output.phong_ads_light = output.phong_ads_light + ambient + diffuse + specular;"                            \
        "       }"                                                                                                              \
        "   }"                                                                                                                  \
        "   else"                                                                                                               \
        "   {"                                                                                                                  \
    ...
    const char *vertexShaderSourceCode_perPixelLighting = 
        ...
        "   float3 light_direction[3] : light_direction;"                                                                         \
    ...
    for(int i = 0; i < 3; i++)
    {
        light[i].ambient[0] = 0.0f;
        light[i].ambient[1] = 0.0f;
        light[i].ambient[2] = 0.0f;
        light[i].ambient[3] = 1.0f;
...
void Display(void)
{
    ...
    light[0].position[1] = radius * sinf(lightAngle);
    light[0].position[2] = radius * cosf(lightAngle);
    light[0].position[3] = 1.0f;

    light[1].position[0] = radius * sinf(lightAngle);
    light[1].position[1] = 0.0f;
    light[1].position[2] = radius * cosf(lightAngle);
    light[1].position[3] = 1.0f;

    light[2].position[0] = radius * sinf(lightAngle);
    light[2].position[1] = radius * cosf(lightAngle);
