...
- (id)initWithFrame:(CGRect)frame
{
    ...
    kundali_texture = [self loadTextureFromBMPFile:"window.app/Vijay_Kundali.bmp"];
    if(!kundali_texture)
    {
        printf("Error : failed to load Vijay_Kundali.bmp texture.\n");
        [self release];
        return (nil);
    }
    
    stone_texture = [self loadTextureFromBMPFile:"window.app/Stone.bmp"];
    if(!stone_texture)
    {
        printf("Error : failed to load Stone.bmp texture.\n");
        [self release];
        return (nil);
    }
    return (self);
